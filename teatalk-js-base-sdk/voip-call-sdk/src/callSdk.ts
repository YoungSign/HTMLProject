...
import { CinCallPacket } from "./call-packets/CallTransaction";
import { templates as callTemplates } from "./call-packets/call-templates";
...
TemplateManager.loadTemplates(MODULE_ID, callTemplates);
...
let TeatalkCallSdk: any = {
    ...
    init: function (sdkParams: { baseSdk: any }) {
        ...
        // 接口2--获取房间列表
        self.baseSdk.register("getRoomList", function() {
            return self.roomList;
        });

        // 接口3--获取房间信息
        self.baseSdk.register("getRoomInfo", function(params: {
            options: {
                roomId: string
            }
        }) {
        ...
        // 接口4--发送邀请
        self.baseSdk.register("sendInvite", function(params: {
            options: {
                from: number,
                to: number | number[],
                mediaType: number,
            ...
            // 2、发cmp
            let packet = new CinCallPacket(object);
        ...
        // 接口5--发送响铃
        self.baseSdk.register("sendRinging", function(params: {
            options: {
                from: number,
                to: number | number[],
                channelID: string,
        ...
        // 接口6--发送接收邀请
        self.baseSdk.register("sendAcceptInvite", function(params: {
            options: {
                from: number,
                to: number | number[],
                mediaType: number,
        ...
        // 接口12--发送挂断
        self.baseSdk.register("sendHangup", function(params: {
            options: {
                from: number,
                to: number | number[],
                mediaType: number,
        ...
        // 接口14--接收各种消息
        self.baseSdk.register("initCall", function(params: { options: object, callback: any }) {
            let callback = params.callback;
        ...
        self.baseSdk.addBinder({
            ...
            binder: function(session: any, moduleType: string, serverPublish: any) {
                console.log("收到音视频消息", serverPublish);
    ...
    onMessage: function (roomId, messageType, message?) {
        ...
        for (let i = 0; i < self.listener.length; i++) {
            ...
            listener(messageType, message, room);
        }
    },
